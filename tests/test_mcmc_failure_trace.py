...
class SourceTraceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.history_path = os.path.join(self.temp_dir.name, "history.npz")

        winds = np.full((7, 500), np.nan, dtype=np.float32)
        winds[6, :289] = np.linspace(-0.5, 0.5, 289, dtype=np.float32)
        episode_lengths = np.zeros(7, dtype=np.int32)
        episode_lengths[6] = 289
        victim_failed = np.zeros(7, dtype=bool)
        victim_failed[6] = True
        failure_steps = np.full(7, -1, dtype=np.int32)
        failure_steps[6] = 289
        ...
        np.savez_compressed(
            ...
            max_wind=np.asarray(1.0, dtype=np.float32),
            wind_sigma=np.asarray(1.0, dtype=np.float32),
            horizon=np.asarray(500, dtype=np.int32),
        )
